...
export async function getGolferScores(tourneyId: number, supabase: SupabaseClient): Promise<GolferScore[]> {
  const result = await supabase.from(GOLFER_SCORE_TABLE).select('*').eq('tourneyId', tourneyId);
  if (result.error) {
...
export async function getGolferScoreOverrides(
  tourneyId: number,
  supabase: SupabaseClient,
): Promise<GolferScoreOverride[]> {
  const result = await supabase.from(GOLFER_SCORE_OVERRIDE_TABLE).select('*').eq('tourneyId', tourneyId);
  if (result.error) {
    console.dir(result.error);
...
export async function updateScores(scores: GolferScore[]): Promise<void> {
  ...
  const result = await adminSupabase().from(GOLFER_SCORE_TABLE).upsert(dbScores, { onConflict: 'tourneyId, golferId' });
  if (result.error) {
    console.dir(result.error);
