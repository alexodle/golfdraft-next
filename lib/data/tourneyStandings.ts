...
export type TourneyStandings = ModelTourneyStandings &
  Readonly<{
...
export function useTourneyStandings(): UseQueryResult<TourneyStandings> {
  ...
  useSharedSubscription<TourneyStandings>(
    TOURNEY_STANDINGS_TABLE,
...
export async function getTourneyStandings(tourneyId: number, supabase: SupabaseClient): Promise<TourneyStandings> {
  ...
  const {
    worstScoresForDay,
    tourney_standings_player_scores: dbPlayerScores,
    ...tourneyStandings
  } = result.data as DbTourneyStandings & { tourney_standings_player_scores: DbTourneyStandingPlayerScore[] };

  const standings = dbPlayerScores
    .map<TourneyStandingPlayerScore>((s) => ({
      ...s,
...
export async function updateTourneyStandings(tourneyStanding: TourneyStandings): Promise<void> {
  ...
  const [result1, result2] = await Promise.all([
    adminSupabase().from(TOURNEY_STANDINGS_TABLE).upsert(dbTourneyStandings),
    adminSupabase().from(TOURNEY_STANDINGS_PLAYER_SCORES_TABLE).upsert(playerScores),
  ]);
